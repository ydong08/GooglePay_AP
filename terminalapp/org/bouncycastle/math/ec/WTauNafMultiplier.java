...
public class WTauNafMultiplier extends ECMultiplier {
    protected ECPoint multiplyPositive(ECPoint eCPoint, BigInteger bigInteger) {
        if (eCPoint instanceof F2m) {
            ...
            byte mu = f2m2.getMu();
            return multiplyWTnaf(f2m, Tnaf.partModReduction(bigInteger, m, byteValue, f2m2.getSi(), mu, (byte) 10), f2m2.getPreCompInfo(f2m, "bc_wtnaf"), byteValue, mu);
        }
    ...
    private static F2m multiplyFromWTnaf(F2m f2m, byte[] bArr, PreCompInfo preCompInfo) {
        ...
        for (int length = bArr.length - 1; length >= 0; length--) {
            ...
            if (b != (byte) 0) {
                if (b > (byte) 0) {
                    f2m3 = f2m3.addSimple(preComp[b]);
                } else {
                    f2m3 = f2m3.subtractSimple(preComp[-b]);
                }
            }
        }
        return f2m3;
