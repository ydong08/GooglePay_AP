...
public class SmartTap2ECKeyManager {
    ...
    public ECPrivateKey decodePrivateKey(byte[] bArr) throws ValuablesCryptoException {
        return privateKeyFromSpec(new ECPrivateKeySpec(new BigInteger(1, bArr), CryptoClient.EC_SPEC));
    }
    ...
    public String hexRepresentation(ECPrivateKey eCPrivateKey) {
        byte[] toByteArray = eCPrivateKey.getS().toByteArray();
        if (toByteArray.length > 32) {
            if (toByteArray.length > 33 || toByteArray[0] != (byte) 0) {
                throw new IllegalArgumentException("Somehow got too many bytes from private key");
            }
            toByteArray = Arrays.copyOfRange(toByteArray, 1, 33);
        }
        if (toByteArray.length < 32) {
            toByteArray = Bytes.concat(new byte[(32 - toByteArray.length)], toByteArray);
        }
