...
public class CoordinatorLayout extends ViewGroup implements NestedScrollingParent {
    ...
    protected void onMeasure(int i, int i2) {
        ...
        setMeasuredDimension(ViewCompat.resolveSizeAndState(suggestedMinimumWidth, i, -16777216 & i5), ViewCompat.resolveSizeAndState(suggestedMinimumHeight, i2, i5 << 16));
    }
            ...
            if (childAt.getVisibility() == 8) {
                ...
                if (layoutParams.isNestedScrollAccepted()) {
                    ...
                    if (behavior != null) {
                        ...
                        if (i > 0) {
                            i6 = Math.max(i3, this.mTempIntPair[0]);
                        } else {
                            i6 = Math.min(i3, this.mTempIntPair[0]);
                        }
                        if (i2 > 0) {
                            max = Math.max(i4, this.mTempIntPair[1]);
                        } else {
                            max = Math.min(i4, this.mTempIntPair[1]);
                        }
                        i7 = max;
                        max = 1;
                    } else {
