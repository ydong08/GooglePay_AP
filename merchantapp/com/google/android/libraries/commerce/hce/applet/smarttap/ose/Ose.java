...
public class Ose {
    ...
    public static byte[] compose(TransactionalDetails transactionalDetails, Optional<ByteArrayWrapper> optional, Iterable<AidInfo> iterable) throws BasicTlvException {
        Collection arrayList = new ArrayList();
        for (AidInfo toTlv : iterable) {
            arrayList.add(toTlv.toTlv());
        }
        BasicTlv addChild = BasicConstructedTlv.getInstance(165).addChild(BasicConstructedTlv.getInstance(48908).addAll(arrayList));
        BasicTlv instance = BasicPrimitiveTlv.getInstance(80, APPLICATION_LABEL);
        BasicTlv instance2 = BasicPrimitiveTlv.getInstance(192, APPLICATION_VERSION);
        BasicConstructedTlv addChild2 = BasicConstructedTlv.getInstance(111).addChild(instance).addChild(instance2).addChild(BasicPrimitiveTlv.getInstance(193, transactionalDetails.toBytes()));
        if (optional.isPresent()) {
            addChild2.addChild(BasicPrimitiveTlv.getInstance(194, ((ByteArrayWrapper) optional.get()).array()));
        }
        addChild2.addChild(addChild);
        return Bytes.concat(addChild2.toByteArray(), Iso7816StatusWord.NO_ERROR.toBytes());
    }
}
