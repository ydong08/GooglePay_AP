...
public abstract class SmartTap2ProprietaryData {
    ...
    BasicTlv toTlv() throws BasicTlvException {
        BasicTlv instance = BasicConstructedTlv.getInstance(115);
        if (minVersion().isPresent()) {
            instance.addChild(BasicPrimitiveTlv.getShortInstance(57197, ((Short) minVersion().get()).shortValue()));
        }
        if (maxVersion().isPresent()) {
            instance.addChild(BasicPrimitiveTlv.getShortInstance(57165, ((Short) maxVersion().get()).shortValue()));
        }
        if (mobileDeviceNonce().isPresent()) {
            instance.addChild(BasicPrimitiveTlv.getInstance(57198, ((ByteArrayWrapper) mobileDeviceNonce().get()).array()));
        }
        instance.addChild(BasicPrimitiveTlv.getByteInstance(57186, (byte) ((supportsSkippingSelect() ? 1 : 0) | 2)));
        return instance;
    }
}
